...
import _okhttp3.OkHttpClient;
import _okhttp3.Request;
import _okhttp3.Response;
import _okhttp3.WebSocket;
import _okhttp3.WebSocketListener;
...
public class RNWebSocketModule extends ReactContextBaseJavaModule {
  ...
  public RNWebSocketModule(ReactApplicationContext context) {
    super(context);
  ...
  @ReactMethod
  public void send(String message, int id, boolean mask) {
    WebSocket client = mWebSocketConnections.get(id);
    if (client == null) {
  ...
  @ReactMethod
  public void sendBinary(String base64String, int id, boolean mask) {
    WebSocket client = mWebSocketConnections.get(id);
    if (client == null) {
      // This is a programmer error
      throw new RuntimeException("Cannot send a message. Unknown WebSocket id " + id);
